    ...
    <div className="flex flex-col h-[100dvh] w-full bg-[#0A0A0A] text-white font-sans overflow-hidden">
        <header className="flex items-center justify-between px-4 md:px-8 h-16 border-b border-white/5 bg-[#0D0D0D] shrink-0 z-10 relative shadow-md">
          <div className="flex items-center gap-3 hidden sm:flex">
             <div className={`w-3 h-3 rounded-full ${isOffline ? 'bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]'}`}></div>
          ...
          <h1 className="text-lg font-light tracking-[0.2em] uppercase italic justify-self-center mx-auto sm:mx-0">Libras<span className="font-bold">Lens</span></h1>
          <div className="flex gap-6 items-center">
             <div className="flex flex-col items-end">
               <span className="text-[10px] uppercase text-white/40 tracking-wider">Engine</span>
               <span className="text-[10px] font-mono text-emerald-500 flex items-center gap-1">
                 {isModelLoading ? <><Loader2 className="w-3 h-3 animate-spin"/> Loading...</> : 'TFJS Active'}
               </span>
             </div>
          </div>
        </header>
          ...
          <main className="flex-[2] relative rounded-3xl bg-black border border-white/5 overflow-hidden group shadow-2xl flex items-center justify-center min-h-[40vh] lg:min-h-0">
             <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
               ...
               <div className="absolute w-[60%] h-[80%] max-w-sm border-2 border-dashed border-emerald-500/30 rounded-2xl flex items-center justify-center hidden sm:flex">
                 <span className="text-[10px] uppercase tracking-[0.3em] text-emerald-500/50">Área de Foco</span>
               </div>
             </div>

             <div className="absolute top-6 left-6 flex gap-2 z-20 pointer-events-none">
               <span className="px-3 py-1 rounded-full bg-black/60 backdrop-blur-md border border-white/10 text-[10px] uppercase tracking-wider text-white/70">Câmera Frontal</span>
             </div>
             ...
             {activeGesture && (
                <div className="absolute bottom-6 right-6 flex flex-col items-center gap-2 z-20 pointer-events-none">
                  ...
                  <span className="text-[10px] uppercase text-emerald-500/60 tracking-widest hidden md:block">Sinal</span>
                </div>
             )}
          ...
          <aside className="w-full lg:flex-1 lg:max-w-md flex flex-col gap-4">
             <div className="flex flex-col h-full bg-[#111111] rounded-3xl border border-white/5 p-6 shadow-2xl">
                <div className="flex justify-between items-center mb-6">
                   <h2 className="text-xs uppercase tracking-[0.2em] text-white/50 flex items-center gap-2">Histórico Recente</h2>
                   <button onClick={() => setHistory([])} className="text-[10px] text-white/20 hover:text-white/60 uppercase tracking-widest transition-colors">Limpar</button>
                </div>
                <div className="flex-1 overflow-y-auto space-y-4 pr-2 pb-4" style={{scrollbarWidth: 'none'}}>
                   {history.length === 0 ? (
                     ...
                     history.map(record => (
                       <div key={record.id} className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 group relative">
                          <p className="text-sm text-white/90 leading-relaxed italic">"{record.text}"</p>
                          <span className="text-[10px] text-white/30 uppercase mt-2 block">
                            {new Date(record.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </span>
                          <button onClick={() => deleteRecord(record.id)} className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 text-[10px] uppercase tracking-widest text-red-500/70 hover:text-red-500 transition-opacity">
                            Excluir
                          </button>
                       </div>
                     ))
        ...
        <section className="h-auto lg:h-32 px-4 md:px-6 pb-4 md:pb-6 shrink-0 relative z-20">
           <div className="h-full w-full bg-[#161616] rounded-[2rem] border border-white/5 flex flex-col lg:flex-row items-center px-6 lg:px-10 py-6 lg:py-0 relative overflow-hidden gap-6 shadow-2xl">
              ...
              <div className="flex-1 w-full flex flex-col justify-center">
                 <span className="text-[10px] uppercase tracking-[0.4em] text-emerald-500 mb-2 block">Tradução em Tempo Real</span>
                 <div className="flex items-center gap-4">
                    <h3 className="text-3xl sm:text-4xl font-light tracking-tight min-h-[2.5rem] break-all">
                       {currentSentence || <span className="text-white/20 italic text-xl sm:text-2xl">Aguardando...</span>}
                       <span className="inline-block font-mono text-white/20 animate-pulse ml-2 -translate-y-1">|</span>
                    </h3>
                    {currentSentence && (
                      <button onClick={() => setCurrentSentence('')} className="shrink-0 p-2 text-white/30 hover:text-red-400 hover:bg-white/5 rounded-full transition-colors flex items-center justify-center">
